...
static int ordinary_ubuntu_runner(const char *value) {
    return strncmp(value, "ubuntu-", 7) == 0;
}

static int exception_runner(const char *value) {
    return strncmp(value, "windows-", 8) == 0 ||
    ...
    while ((len = getline(&line, &cap, file)) >= 0) {
        ...
        if (count != 4 || *trim(fields[0]) == '\0' || *trim(fields[1]) == '\0' ||
            *trim(fields[2]) == '\0' || *trim(fields[3]) == '\0' ||
            !exception_runner(trim(fields[2]))) {
            fail(result, "GITHUB-EXCEPTION-MALFORMED", relative, NULL,
                 "expected workflow, job, concrete non-Ubuntu hosted runs_on, reason");
            continue;
        }
...
static void finish_job(Result *result, Exception *exceptions,
    ...
    if (job->ordinary_ubuntu) return;
    if (job->exception_hosted) {
    ...
    while ((len = getline(&line, &cap, file)) >= 0) {
        ...
        if (job.child_indent_set && indent == job.child_indent &&
            ...
            if (*value == '\0') {
                ...
                job.ordinary_ubuntu = ordinary_ubuntu_runner(scalar);
