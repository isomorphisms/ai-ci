...
For the Blackball evaluation, `evals/blackball/verdicts.tsv` owns the canonical `classification`. The fixtures in `evals/blackball/mocks/cases.jsonl` separately retain `substantive_effect`, `evidence_status`, and `run_status`. Those fields are diagnostic dimensions, not alternate verdict names.
