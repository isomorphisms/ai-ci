...
| Infrastructure-stage conflation | Treated a runner, secret, upload, publication, reporting, or CI-service failure as proof that separately tested implementation logic was wrong, or treated passing implementation tests as proof that the failed delivery stage was healthy. | Shared agent guidance now requires stage-specific classification. Existing staged receipts preserve separate results where modeled; a generic executable classifier is not yet claimed. |
...
| Merge authority manufactured from an acknowledgement | The earlier ledger treated Cat Food #38’s “Okay” as intrinsically non-authorizing. The actual failure class is narrower: an acknowledgement must not create merge authority when prior task context did not authorize merging; when that context already authorizes merge as a possible completion, ambiguous continuation is valid. | PR #141, “Preserve contextual merge authority,” accepts exact-state `task-context` authority, rejects isolated acknowledgement as a source of authority, and keeps blockers, objections, intent/scope changes, and stale exact-state evidence fail-closed. |
...
| 2026-09-18 | Exact merged `isomorphisms/bioawk` PR #4, “Add deterministic FASTA/FASTQ motif oracle” | The oracle itself correctly tested per-record overlapping matches, wrapped FASTA sequence handling, a FASTQ quality decoy, and cross-record isolation, but its workflow path filter named only the oracle directory and workflow file. A later production parser or sequence-field change therefore could bypass the semantic check entirely. The shared `workflow-integrity-v0` contract already has event-scoped critical-path enforcement; the durable rule is to keep that reusable mechanism in `ai-ci` and either enumerate all material consumer dependencies or run the semantic workflow unfiltered. |
